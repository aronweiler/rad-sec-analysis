...
class AgenticStageBase(StageBase, ErrorRecoveryMixin, ABC):
    ...
    def __init__(
        ...
        self.context_window_manager = ContextWindowManager(
            self.token_counter, self.llm_factory
        )
    ...
    async def execute_agentic_workflow(self, **kwargs) -> Any:
        ...
        await self.tool_manager.initialize_tools(
            self.stage_config, self.get_required_tools()
        )
        ...
        result = await self.loop_controller.execute_loop(
            ...
            termination_checker=lambda resp, term_result: self._should_terminate(
                resp, term_result, **kwargs
            ),
            forced_termination_handler=lambda: self._handle_forced_termination(
                **kwargs
            ),
