...
class CVEAnalysis(BaseModel):
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "example": {
                ...
                "affected_software": ["Apache Tomcat 9.0.50", "MySQL Connector/J 8.0.25"],
    ...
    exploitation_likelihood: ExploitationLikelihood = Field(..., description="Likelihood of exploitation in this incident")
    relevance_to_incident: float = Field(..., description="Relevance score (0-10) to this specific incident")
    affected_software: List[str] = Field(default_factory=list, description="Software affected by this CVE")
    attack_vector_alignment: str = Field(..., description="How this CVE aligns with observed attack vectors")
    contextual_risk_assessment: str = Field(..., description="Risk assessment specific to this incident context")
    exploitation_evidence: Optional[str] = Field(None, description="Evidence suggesting this CVE was exploited")
    mitigation_priority: int = Field(..., description="Priority for mitigation (1-10, 10 being highest)")
...
class AssetRiskAssessment(BaseModel):
    ...
    overall_risk_level: RiskLevel = Field(..., description="Overall risk level for this asset")
    vulnerability_count: int = Field(..., description="Number of vulnerabilities affecting this asset")
    critical_vulnerabilities: List[str] = Field(default_factory=list, description="Critical CVEs affecting this asset")
    network_exposure: str = Field(..., description="Assessment of network exposure")
    business_impact_potential: str = Field(..., description="Potential business impact if compromised")
    recommended_actions: List[str] = Field(default_factory=list, description="Specific actions for this asset")
    compromise_indicators: List[str] = Field(default_factory=list, description="Indicators suggesting compromise")
...
class TTPAnalysis(BaseModel):
    ...
    relevance_to_vulnerabilities: str = Field(..., description="How this TTP relates to identified vulnerabilities")
    attack_stage: str = Field(..., description="Stage in the attack lifecycle")
    supporting_cves: List[str] = Field(default_factory=list, description="CVEs that could enable this TTP")
    defensive_gaps: List[str] = Field(default_factory=list, description="Defensive gaps this TTP exploits")
    detection_opportunities: List[str] = Field(default_factory=list, description="Opportunities for detection")
...
class VulnerabilityChain(BaseModel):
    ...
    likelihood: ExploitationLikelihood = Field(..., description="Likelihood of this chain being exploited")
    impact_assessment: str = Field(..., description="Potential impact if this chain is exploited")
    supporting_evidence: List[str] = Field(default_factory=list, description="Evidence supporting this chain")
...
class ActionableRecommendation(BaseModel):
    ...
    category: str = Field(..., description="Category (e.g., 'Immediate', 'Short-term', 'Long-term')")
    action: str = Field(..., description="Specific action to take")
    rationale: str = Field(..., description="Reasoning behind this recommendation")
    affected_assets: List[str] = Field(default_factory=list, description="Assets this recommendation applies to")
    related_cves: List[str] = Field(default_factory=list, description="CVEs this recommendation addresses")
...
class IncidentAnalysisResult(BaseModel):
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "example": {
                ...
                "asset_risk_assessments": [
                    {
                        ...
                        "recommended_actions": ["Immediate patching", "Network segmentation"],
                        "compromise_indicators": ["Unusual network traffic", "Suspicious processes"]
                    }
                ],
                ...
                "ttp_analysis": [
                    {
                        ...
                        "detection_opportunities": ["Monitor failed logins", "Behavioral analysis"]
                    }
                ],
                ...
                "environmental_factors": ["Internet-facing services", "Weak authentication policies"],
                ...
                "data_sources_used": ["Incident report", "Vulnerability database", "Network logs"],
                "limitations_and_assumptions": ["Limited log retention", "Assumed standard network architecture"],
                "follow_up_investigations": ["Deep packet inspection", "Memory forensics", "Timeline analysis"]
            }
        }
    )
    ...
    analyst_confidence: float = Field(..., description="Confidence level in analysis (0-10)")
    
    # Executive Summary
    executive_summary: str = Field(..., description="High-level summary of findings")
    overall_risk_assessment: RiskLevel = Field(..., description="Overall incident risk level")
    ...
    # Asset Analysis
    asset_risk_assessments: List[AssetRiskAssessment] = Field(default_factory=list, description="Risk assessment for each affected asset")
    most_critical_assets: List[str] = Field(default_factory=list, description="Hostnames of most critical assets")
    
    # TTP Analysis
    ttp_analysis: List[TTPAnalysis] = Field(default_factory=list, description="Analysis of observed TTPs")
    attack_progression: str = Field(..., description="Assessment of attack progression and timeline")
    
    # Attack Chain Analysis
    potential_attack_chains: List[VulnerabilityChain] = Field(default_factory=list, description="Potential vulnerability exploitation chains")
    most_likely_attack_path: Optional[str] = Field(None, description="Most likely attack path based on evidence")
    
    # Intelligence and Context
    threat_actor_assessment: Optional[str] = Field(None, description="Assessment of potential threat actor characteristics")
    environmental_factors: List[str] = Field(default_factory=list, description="Environmental factors affecting the incident")
    detection_gaps: List[str] = Field(default_factory=list, description="Identified detection gaps")
    
    # Recommendations
    immediate_actions: Optional[List[ActionableRecommendation]] = Field(default_factory=list, description="Immediate response actions")
    short_term_recommendations: Optional[List[ActionableRecommendation]] = Field(default_factory=list, description="Short-term improvements")
    long_term_recommendations: Optional[List[ActionableRecommendation]] = Field(default_factory=list, description="Long-term strategic recommendations")
    
    # Supporting Information
    reasoning_chain: List[str] = Field(default_factory=list, description="Step-by-step reasoning process")
    data_sources_used: List[str] = Field(default_factory=list, description="Data sources consulted during analysis")
    limitations_and_assumptions: List[str] = Field(default_factory=list, description="Analysis limitations and assumptions")
    follow_up_investigations: List[str] = Field(default_factory=list, description="Recommended follow-up investigations")