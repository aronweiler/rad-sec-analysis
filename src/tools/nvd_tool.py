...
@dataclass
class SoftwareVulnerabilityReport:
...
@dataclass
class IncidentVulnerabilityReport:
    ...
    most_vulnerable_software: Optional[SoftwareInfo]
...
class NVDTool:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if self.api_key:
            ...
            logging.warning("No NVD API key provided. Rate limited to 10 requests per minute.")
    ...
    def search_cves_by_keyword(self, keyword: str, results_per_page: int = 20) -> List[CVEInfo]:
        """
    ...
    def search_cves_by_cpe(self, cpe_name: str, results_per_page: int = 20) -> List[CVEInfo]:
        """
    ...
    def search_cves_by_date_range(self, start_date: datetime, end_date: datetime, 
                                  results_per_page: int = 20) -> List[CVEInfo]:
        """
    ...
    def analyze_software_vulnerabilities(self, software: SoftwareInfo, 
                                    max_results: int = 50) -> SoftwareVulnerabilityReport:
        """
        ...
        """
        self.logger.info(f"Analyzing vulnerabilities for {software.name} {software.version}")
        ...
        for strategy_name, search_terms in search_strategies.items():
            ...
            for term in search_terms:
                ...
                cves = self.search_cves_by_keyword(term, min(max_results - len(all_cves), 50))
                ...
                self.logger.debug(f"Found {len(cves)} CVEs, {relevant_count} relevant for '{term}'")
        ...
        if len(all_cves) < 10:
            ...
            for cpe_term in cpe_searches:
                if len(all_cves) >= max_results:
                    break
                cves = self.search_cves_by_keyword(cpe_term, min(max_results - len(all_cves), 25))
                for cve in cves:
                    if cve.cve_id not in seen_cve_ids and self._is_cve_relevant_to_software(cve, software):
                        all_cves.append(cve)
                        seen_cve_ids.add(cve.cve_id)

        # Count vulnerabilities by severity
        critical_count = sum(1 for cve in all_cves if cve.cvss_v3_severity == 'CRITICAL')
        ...
        self.logger.info(f"Found {len(all_cves)} relevant CVEs for {software.name} {software.version}")
    ...
    def _generate_search_strategies(self, software: SoftwareInfo) -> Dict[str, List[str]]:
        """
        ...
        if version and version.lower() not in ['unknown', 'n/a', '']:
            ...
            major_version = version.split('.')[0] if '.' in version else version.split()[0]
            version_terms.extend([
                f"{core_name} {major_version}",
                f"{name} {major_version}"
            ])
    ...
    def _extract_core_product_name(self, name: str) -> str:
        ...
        prefixes_to_remove = [
            'microsoft ', 'adobe ', 'oracle ', 'apache ', 'cisco ', 
            'vmware ', 'red hat ', 'redhat ', 'ibm ', 'google ',
            'amazon ', 'aws ', 'open source '
        ]
        ...
        suffixes_to_remove = [
            ' server', ' client', ' enterprise', ' professional', 
            ' standard', ' community', ' edition', ' software',
            ' application', ' service', ' platform', ' framework'
        ]
    ...
    def _extract_vendor_product(self, name: str) -> List[str]:
        ...
        vendor_mappings = {
            ...
            'microsoft': ['microsoft', 'windows', 'office', 'exchange', 'sql server', 'iis'],
    ...
    def analyze_incident_vulnerabilities(self, incident: IncidentData, 
                                       max_cves_per_software: int = 25) -> IncidentVulnerabilityReport:
        """
        ...
        """
        self.logger.info(f"Analyzing vulnerabilities for incident {incident.incident_id}")
        ...
        for software in unique_software:
            report = self.analyze_software_vulnerabilities(software, max_cves_per_software)
    ...
    def get_recent_cves(self, days: int = 7, results_per_page: int = 50) -> List[CVEInfo]:
        """
    ...
    def _is_cve_relevant_to_software(self, cve: CVEInfo, software: SoftwareInfo) -> bool:
        """
    ...
    def _generate_recommendations(self, software_reports: List[SoftwareVulnerabilityReport], 
                                incident: IncidentData) -> List[str]:
        """
        ...
        if public_assets and any(r.critical_count > 0 or r.high_count > 0 for r in software_reports):
            recommendations.append(
    ...
    def export_report_to_dict(self, report: IncidentVulnerabilityReport) -> Dict[str, Any]:
        """
        Export vulnerability report to dictionary format
        ...
        return {
            ...
            'summary': {
                ...
                } if report.most_vulnerable_software else None
            },
            'software_reports': [
                {
                    'software': {
                        'name': sr.software.name,
                        'version': sr.software.version
                    },
                    ...
                    'cves': [
                        {
                            ...
                            'references': cve.references[:3]  # Limit to first 3 references
                        }
                        for cve in sr.cves[:10]  # Limit to first 10 CVEs per software
                    ]
                }
                for sr in report.software_reports
            ],
